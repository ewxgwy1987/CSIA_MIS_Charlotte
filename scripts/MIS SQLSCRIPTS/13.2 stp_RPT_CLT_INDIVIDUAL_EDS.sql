...
BEGIN
	...
	WHERE	MSTAT.STATUS_ID LIKE '%NAME' AND LOCATION=@SUBSYSTEM AND MSTAT='EDS';
	...
	FROM	MDS_STATUS MSTAT
	WHERE	LTRIM(RTRIM(MSTAT.STATUS))='SW_TYPE' AND MSTAT.LOCATION=@SUBSYSTEM;

	--3. UPDATE SW_REV
	UPDATE	#INDIV_EDS_STATUS
	SET		SW_REV=LTRIM(RTRIM(MSTAT.STATUS))
	FROM	MDS_STATUS MSTAT
	WHERE	LTRIM(RTRIM(MSTAT.STATUS))='SW_VER' AND MSTAT.LOCATION=@SUBSYSTEM;

	--4. UPDATE KEY_POS
	UPDATE	#INDIV_EDS_STATUS
	SET		KEY_POS=LTRIM(RTRIM(MSTAT.STATUS))
	FROM	MDS_STATUS MSTAT
	WHERE	LTRIM(RTRIM(MSTAT.STATUS))='KEY_POS' AND MSTAT.LOCATION=@SUBSYSTEM;

	--5. UPDATE EDS_STATUS
	DECLARE @STATUS INT;
	SELECT	@STATUS=CAST(LTRIM(RTRIM(MSTAT.STATUS)) AS INT)
	FROM	MDS_STATUS MSTAT
	WHERE	LTRIM(RTRIM(MSTAT.STATUS))='STATUS' AND MSTAT.LOCATION=@SUBSYSTEM;

	UPDATE	#INDIV_EDS_STATUS
	SET		EDS_STATUS=(SELECT DESCRIPTION FROM MDS_STATUS_LK WHERE MIN<=@STATUS AND @STATUS<ISNULL(MAX,65535))
	
	--6. UPDATE PLC_SCAN_TIME
	DECLARE @PLC_NAME VARCHAR(50);
	DECLARE @PLC_SCANTIME VARCHAR(50);

	SELECT	@PLC_NAME=LTRIM(RTRIM(MSTAT.STATUS))
	...
	SELECT	@PLC_SCANTIME=LTRIM(RTRIM(MSTAT.STATUS))
	FROM	MIS_DEVICE_PLC_MAP DPM, MDS_STATUS MSTAT
	WHERE	DPM.EQUIP_SUBSYSTEM=@SUBSYSTEM 
		AND DPM.PLC_OTHERNAME=MSTAT.LOCATION
		AND MSTAT.TYPE='PLC'
		AND	MSTAT.STATUS_ID LIKE '%SCANTTIME';

	UPDATE	#INDIV_EDS_STATUS
	SET		EDS_STATUS=@PLC_SCANTIME+'ms ('+@PLC_NAME+')';
