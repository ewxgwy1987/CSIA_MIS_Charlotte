...
ALTER PROCEDURE dbo.stp_RPT_CLT_LOADBALANCING_NORMAL
		  @DTFROM DATETIME,
		  @DTTO DATETIME,
...
BEGIN
	...
	WHERE  (SC.DETECT_LOCATION IS NOT NULL OR SC.DETECT_LOCATION<>'')
		...
		AND SC.SUBSYSTEM IN (SELECT * FROM DBO.RPT_GETPARAMETERS(@SUBSYSTEM))
		--AND SC.MDS_DATA=0
	...
	WHERE MBCR.LOCATION=SC.DETECT_LOCATION
