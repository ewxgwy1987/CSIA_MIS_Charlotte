...
ALTER PROCEDURE dbo.stp_RPT_CLT_DAYEND_INPUT
		  @DTFrom DATETIME,
		  @DTTo DATETIME
AS
BEGIN
	...
	SELECT MBCR.SUBSYSTEM AS 'LOCATION',SUM(MBC.DIFFERENT) AS 'COUNT', 
		CASE 
			WHEN MBCR.SUBSYSTEM LIKE '%TC%' THEN 'TC'
			WHEN MBCR.SUBSYSTEM LIKE '%IC%' THEN 'IC'
			WHEN MBCR.SUBSYSTEM LIKE '%CS%' THEN 'CS'
			WHEN MBCR.SUBSYSTEM LIKE '%OS%' THEN 'OS'
		END AS INPUT_GROUP
...
USE [BHSDB];
GO
ALTER PROCEDURE dbo.stp_RPT_CLT_DAYEND_ATRSTATS
		  @DTFrom DATETIME,
		  @DTTo DATETIME,
...
USE [BHSDB];
GO
ALTER PROCEDURE dbo.stp_RPT_CLT_DAYEND_BDDSTATS
		  @DTFrom DATETIME,
		  @DTTo DATETIME
AS
BEGIN
...
USE [BHSDB];
GO
ALTER PROCEDURE dbo.stp_RPT_CLT_DAYEND_EDSSTATS
		  @DTFrom DATETIME,
		  @DTTo DATETIME
AS
BEGIN
...
USE [BHSDB];
GO
ALTER PROCEDURE dbo.stp_RPT_CLT_DAYEND_OUTPUT
		  @DTFrom DATETIME,
		  @DTTo DATETIME
AS
BEGIN
...
EXEC stp_RPT_CLT_TRACKEDPHOTOCELL @DTFrom,@DTTo,@SUBSYSTEM;
