...
--11	Machine Alarm / Operator Alarm		MAOA
--12	Machine Alarm / Operator Clear		MAOC
--13	Machine Alarm / Operator Unknown	MAOU
--14	Machine Alarm / Operator Pending	MAOP
--15	Machine Alarm / Operator Timeout	MAOT
--21	Machine Clear / Operator Alarm		MCOA
--22	Machine Clear / Operator Clear		MCOC
--23	Machine Clear / Operator Unknown	MCOU
--24	Machine Clear / Operator Pending	MCOP
--25	Machine Clear / Operator Timeout	MCOT
...
--X		Error / Unknown						ERROR
	...
	SELECT DISTINCT ICR.GID, 
			...
			-----Machine Flag
			CASE 
				WHEN ICR.RESULT_TYPE!='33' AND ICR.RESULT_TYPE!='X' THEN 1
				ELSE 0
			END	AS Machine_Decision_Flag,

			CASE 
				WHEN ICR.RESULT_TYPE LIKE '2%' THEN 1
				ELSE 0
			END	AS Machine_Clear_Flag,

			CASE 
				WHEN ICR.RESULT_TYPE LIKE '1%' THEN 1
				ELSE 0
			...
			CASE 
				WHEN ICR.RESULT_TYPE NOT LIKE '%2' THEN 1
				ELSE 0
	...
	FROM (
			...
			WHERE IPR.LOCATION=LOC.LOCATION_ID AND IPR.PROCEED_LOCATION=PRDLOC.LOCATION_ID
				AND ELD.DIVERT_LOCATION=LOC.LOCATION
				AND ELD.CLEAR_LOCATION=PRDLOC.LOCATION
			...
			UNION ALL
			SELECT IPR.GID, PRDLOC.LOCATION AS PRDLOC, IPR.TIME_STAMP, 'C' AS IPRTYPE
			FROM ITEM_PROCEEDED IPR, LOCATIONS PRDLOC WITH(NOLOCK)
			WHERE IPR.PROCEED_LOCATION=PRDLOC.LOCATION_ID
				AND PRDLOC.LOCATION IN ('OOG1-15B','OOG2-17B','ED9-33B')
				AND IPR.TIME_STAMP BETWEEN DATEADD(MINUTE,-@MINUTERANGE,@DTFROM) AND DATEADD(MINUTE,@MINUTERANGE,@DTTO)
			--c. AL line going to CBRA
			UNION ALL
			SELECT IPR.GID, PRDLOC.LOCATION AS PRDLOC, IPR.TIME_STAMP, 'A' AS IPRTYPE
			FROM ITEM_PROCEEDED IPR, LOCATIONS PRDLOC WITH(NOLOCK)
			WHERE IPR.PROCEED_LOCATION=PRDLOC.LOCATION_ID
				AND PRDLOC.SUBSYSTEM LIKE 'SB%'
				--AND PRDLOC.LOCATION='AL1-11'
				AND IPR.TIME_STAMP BETWEEN DATEADD(MINUTE,-@MINUTERANGE,@DTFROM) AND DATEADD(MINUTE,@MINUTERANGE,@DTTO)
		) AS ALLIPR;
